    ...
    try {
        ...
        if (data) {
            investments = data.map(i => ({
                ...
                avgPrice: parseFloat(i.avg_price),
                currentPrice: parseFloat(i.current_price)
            }));
        }
    } catch (err) {
        investments = [];
    ...
    try {
        ...
        if (data) {
            debts = data.map(d => ({
                ...
                remainingBalance: parseFloat(d.remaining_balance),
...
function renderInvestments() {
    ...
    if (investments.length === 0) {
        container.innerHTML = `<div class="empty-state"><p>No investments tracked yet.</p></div>`;
        return;
    }
            ...
            <div class="investment-card">
                <div class="asset-header">
                    <div class="asset-info">
                        <h4>${inv.name}</h4>
                        <div class="asset-category">${inv.category}</div>
                    </div>
                    <div class="asset-badge badge-${inv.category.toLowerCase()}">${inv.category}</div>
                </div>
                <div class="asset-values">
                    <div>
                        <div class="val-label">Market Value</div>
                        <div class="val-amount">${fmt(currentVal)}</div>
                    </div>
                    <div>
                        <div class="val-label">Total Cost</div>
                        <div class="val-amount">${fmt(totalCost)}</div>
                    </div>
                </div>
                <div class="profit-loss ${isProfit ? 'text-profit' : 'text-loss'}">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                        <polyline points="${isProfit ? '23 6 13.5 15.5 8.5 10.5 1 18' : '23 18 13.5 8.5 8.5 13.5 1 6'}" />
                        <polyline points="${isProfit ? '17 6 23 6 23 12' : '17 18 23 18 23 12'}" />
                    </svg>
                    ${isProfit ? '+' : ''}${fmt(profit)} (${profitPct.toFixed(2)}%)
                </div>
                <div class="investment-actions">
                    <button class="btn-icon-sm delete" onclick="deleteInvestment('${inv.id}')" title="Delete asset">
...
function renderDebts() {
    ...
    if (debts.length === 0) {
        container.innerHTML = `<div class="empty-state"><p>No debts tracked yet.</p></div>`;
        return;
    }

    container.innerHTML = debts.map(debt => {
        const paidAmount = debt.totalAmount - debt.remainingBalance;
        const progressPct = (paidAmount / debt.totalAmount) * 100;
        ...
        return `
            <div class="debt-card">
                <div class="debt-header">
                    <div class="debt-info">
                        <h4>${debt.lender}</h4>
                        <div class="debt-lender">Interest: ${debt.interestRate}%</div>
                    </div>
                    <div class="asset-badge badge-crypto" style="background:rgba(212,115,90,0.15); color:var(--coral);">Loan</div>
                </div>
                <div class="debt-values">
                    <div>
                        <div class="val-label">Remaining</div>
                        <div class="val-amount" style="color:var(--coral)">${fmt(debt.remainingBalance)}</div>
                    </div>
                    <div>
                        <div class="val-label">Total Loan</div>
                        <div class="val-amount">${fmt(debt.totalAmount)}</div>
                    </div>
                </div>
                <div class="debt-progress-container">
                    <div class="val-label">Repayment Progress: ${progressPct.toFixed(1)}%</div>
                    <div class="debt-progress-bar">
                        <div class="debt-progress-fill" style="width: ${progressPct}%"></div>
                    </div>
                    <div class="debt-meta">
                ...
                <div class="debt-actions">
                    <button class="btn-icon-sm delete" onclick="deleteDebt('${debt.id}')" title="Delete loan">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2" />
                        </svg>
                    </button>
...
function openInvestmentModal() {
    ...
    document.getElementById('invAvgPrice').value = '';
    document.getElementById('invCurrentPrice').value = '';
...
async function addInvestment() {
    ...
    const avgPrice = parseFloat(document.getElementById('invAvgPrice').value);
    const currentPrice = parseFloat(document.getElementById('invCurrentPrice').value);
    const btn = document.getElementById('addInvBtn');

    if (!name || isNaN(quantity) || isNaN(avgPrice) || isNaN(currentPrice)) {
        showToast('Please fill in all fields correctly');
        return;
    }
...
function openDebtModal() {
    document.getElementById('debtLender').value = '';
...
async function addDebt() {
    ...
    if (!lender || isNaN(total) || isNaN(remaining) || isNaN(rate)) {
        showToast('Please fill in all fields correctly');
        return;
    }
